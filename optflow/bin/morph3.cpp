...
int main(int argc, char **argv)
{
  ...
  reqArgs.add_options()
    ...
    ("vec1", value< std::string >(), "first vector field")
    ("vec2", value< std::string >(), "second vector field")
  ...
  try {
    ...
    else if(!vm.count("image1") || !vm.count("image2") || !vm.count("vec1") || !vm.count("vec2") || !vm.count("numtimesteps") || 
    ...
    std::string vec1FileName = vm["vec1"].as< std::string >();
    std::string vec2FileName = vm["vec2"].as< std::string >();
    ...
    if(algorithmName == "proesmans")
    {
      ...
      DenseVectorFieldIO::readVectorField(vec1FileName,V1);
      DenseVectorFieldIO::readVectorField(vec2FileName,V2);
