...
void PyramidalDenseMotionExtractor::compute(const CImg< unsigned int > &I1,
...
void PyramidalDenseMotionExtractor::compute(const CImg< unsigned int > &I1,
                                            ...
                                            CImg< double > &VB)
{
  std::cout << "pyramidaldencemotion extractor compute\n";
  const int W = I1.dimx();
  const int H = I1.dimy();
                                                  ...
                                                  CImg< double > &VB)
{
  ...
  curLevelI[0].assign(imagePyramids[0].getImageLevel(level));
  curLevelI[1].assign(imagePyramids[1].getImageLevel(level));
  std::cout << "before assign\n";
  if(isDual()) {
    //dynamic_cast< DualDenseMotionExtractor * >
    std::cout << "compute\n";
    dynamic_cast< DualDenseMotionExtractor * >(motionExtractor)->compute(curLevelI[0], curLevelI[1], VF, VB);
  }
  else {
    std::cout << "without dual\n";
