...
void LucasKanade::compute(const CImg< unsigned int > &I1,
...
void LucasKanade::printInfoText() const
{
  ...
  cout<<"Input image width: "<<I1_.dimx()<<endl;
  cout<<"Input image height: "<<I1_.dimy()<<endl;
...
void LucasKanade::computeGradients_(const CImg< unsigned int > &I,
                                    CImg< double > &G)
{
                                      ...
                                      LSQResults &results)
{
    ...
    for(i = 0; i < NUM_ITERATIONS_; i++)
    {
      ...
      for(yw = -WINDOW_RADIUS_; yw <= WINDOW_RADIUS_; yw++)
      {
        for(xw = -WINDOW_RADIUS_; xw <= WINDOW_RADIUS_; xw++)
        {
          ...
          gxs = G1_.at4(x1Abs, y1Abs, 0, 0);
          gys = G1_.at4(x1Abs, y1Abs, 0, 1);
          
          I1s = I1_.at2(x1Abs, y1Abs) * INTENSITY_SCALE_;
          I2s = I2_.linear_at2(x2Abs, y2Abs) * INTENSITY_SCALE_;
  ...
  results.quality[1] = min(65535.0, 65535.0 / (1000.0*r + 1.0))  / 65535.0;
}
