	...
	for day, hours in cases.items():
		...
		for step in time_intervals:
			...
			for i in range(0,60/step):
				...
				#try to find best observation
				for k in [0,-1,1,2]:
					start_tmp = start+k
					end_tmp = int(end)+k
					print str(start_tmp) + " start time "
					print "pattern start "
					print raws+"RAW/"+site+'/'+day+'/'+str(date)+str(start_tmp)+"*_"+site+"_ppi.raw"
					print "pattern stop"
					print raws+"RAW/"+site+'/'+day+'/'+str(date)+str(end_tmp)+"*_"+site+"_ppi.raw"
					if site != 'VAN':
						try:
